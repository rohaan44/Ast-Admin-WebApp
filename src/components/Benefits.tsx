import { Button } from "@/components/ui/button";
import benefitsImage from "@/assets/benefits-image.png";
import BenefitsList from "@/components/ui/landing_page/benifit_list";
import AppButton from "@/components/ui/landing_page/app_button";

  const items = [ "Piani di allenamento e nutrizione personalizzati", "Integrazioni intelligenti con Apple Health,Google Fit",
     "Check-in basati sull'intelligenza artificiale",
                  "Chat diretta con il tuo coach", "Accesso all'AST® Academy",
                  ];
const Benefits = () => 

  {
  return (
    <section className="section bg-background">
      <div className="container-custom">
        <div className="w-full flex justify-center mb-16">
          <h2 className="text-4xl md:text-5xl lg:text-6xl font-bold mb-8 leading-tight text-center">
            Cosa ottieni con <span className="text-primary">AST®</span>
          </h2>
        </div>
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          {/* Image */}
          <div className="relative fade-in order-2 lg:order-1">
            <div className="relative overflow-hidden rounded-2xl">
              <img
                src={benefitsImage}
                alt="Train better, Grow stronger, Transform yourself with AST"
                className="w-full object-cover hover-lift"
              />
              <div className="absolute inset-0 bg-gradient-to-t from-background/80 to-transparent"></div>
            </div>
          </div>

          {/* Content */}
          <div className="fade-in order-1 lg:order-2">
            <div className="space-y-8 mb-10">
              <div>
                <h3 className="text-5xl md:text-5xl font-bold mb-3">
                  Train better. Grow stronger. Transform yourself with{" "}
                  <span className="text-primary">AST®</span>.
                </h3>

                <p className="text-lg text-muted-foreground leading-relaxed pt-5">
                 
                  <BenefitsList
                    items={items}
                    textClassName="text-muted-foreground text-base"
                  />
                </p>
              </div>
            </div>

            <AppButton
              label="Iscrizione"
              onClick={() => console.log("Desktop pressed")}
            />
          </div>
        </div>
      </div>
    </section>
  );
};

export default Benefits;
