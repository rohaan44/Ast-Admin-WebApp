...
const Workout = () => {
  ...
  const [activeWeek, setActiveWeek] = useState(details.weeks[0]);

  const exerciseDetails = [
    { icon: <FaFire className='text-red-400' />, text: details.kcal },
    { icon: <FaDumbbell />, text: details.allenamento },
    { icon: <FaStar />, text: `${details.valutazioni}` },
    { icon: null, text: details.serie },
    { icon: null, text: details.rir },
  ];
  ...
  return (
    <section className='w-full md:mb-0 md:mt-0 mt-16 mb-20 p-3'>
      ...
      <section className='grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-10 mt-10 mb-5'>
        <div className='flex flex-col gap-10'>
          <div className='relative rounded-2xl overflow-hidden w-full h-[25rem] md:h-[35rem]'>
            <img src={exercise.image} className='w-full h-full object-cover' />
        ...
        <div className='py-4 sm:px-4 px-0 flex flex-col items-start gap-6'>
          ...
          <div>
            <h1 className='text-2xl font-semibold'>{details.mainTitle}</h1>
            <p className='text-lg text-white/80'>{details.mainSubtitle}</p>
          </div>
          ...
          <div>
            <h1 className='text-2xl font-semibold'>{details.infoTitle}</h1>
          ...
          <div className='w-full'>
            <h1 className='text-2xl font-semibold'>
              {details.progressione.title}
            </h1>
            <ul className='flex flex-col gap-2 mt-4'>
              <li className='flex items-center gap-2'>
                <IoCheckmarkDoneCircleOutline className='text-green-500 text-xl' />
                <p>{details.progressione.beginner}</p>
              </li>
              <li className='flex items-center gap-2'>
                <IoCheckmarkDoneCircleOutline className='text-green-500 text-xl' />
                <p>{details.progressione.intermediate}</p>
              </li>
              <li className='flex items-center gap-2'>
                <IoCheckmarkDoneCircleOutline className='text-green-500 text-xl' />
                <p>{details.progressione.advanced}</p>
              </li>
            </ul>
          </div>
          ...
          <div className='w-full'>
            <h1 className='text-2xl font-semibold'>
              {details.technicalNotes.title}
            </h1>
            <ul className='flex flex-col mt-3 gap-1'>
              {details.technicalNotes.notes.map((n, i) => (
                <p key={i}>{n}</p>
              ))}
            </ul>
          </div>
          ...
          <div className='w-full'>
            <h1 className='text-2xl font-semibold'>
              {details.indications.title}
            </h1>
            <ul className='flex flex-col mt-3 gap-1'>
              {details.indications.list.map((n, i) => (
                <p key={i}>{n}</p>
              ))}
            </ul>
          </div>
      ...
      <section className='grid grid-cols-1 lg:grid-cols-2 gap-10 sm:px-4 px-0 py-10'>
        <div>
          <h1 className='text-2xl font-semibold'>{details.weekGoal}</h1>
          <p className='text-sm text-white/70 mt-1'>
            {details.weekDescription}
          </p>
        ...
        <div className='flex flex-col gap-6'>
          <div className='flex items-center flex-wrap gap-1 justify-between'>
            <h1 className='text-2xl font-semibold'>{details.trainer.title}</h1>
              ...
              >
                ...
                <div className='flex items-start justify-start flex-col gap-2'>
                  <h1 className='text-xl font-semibold'>
                    {details.trainer.name}
                  </h1>
                  <ul className='flex flex-wrap items-center gap-2 text-sm'>
                    {details.trainer.categories.map((c, j) => (
                      <li
                        key={j}
